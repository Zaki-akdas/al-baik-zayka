...
import { listActiveOffers, type Offer } from "@/lib/db";
...
export function Offers() {
  const [dbOffers, setDbOffers] = useState<Offer[] | null>(null);
              ...
              <Reveal key={offer.id} delay={i * 0.08}>
                <article className="group flex h-full flex-col overflow-hidden rounded-3xl border border-white/10 bg-white/5 backdrop-blur-sm transition-colors duration-300 hover:border-gold/40">
                  <div className="relative aspect-[16/9] overflow-hidden">
                    ...
                    {offer.price !== null && (
                      <span className="absolute right-3 bottom-3 font-display text-xl text-gold-bright">₹{offer.price}</span>
                    )}
                  </div>
